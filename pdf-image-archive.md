...
核心文件路径：
- Worker 入口：[worker.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/worker/worker.ts)
- 链接处理循环：[linkProcessing.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/worker/workers/linkProcessing.ts)
- 归档主处理器：[archiveHandler.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/worker/lib/archiveHandler.ts)
...
相关文件：
- Tag 判定：[isArchivalTag.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/packages/lib/isArchivalTag.ts)
- User 默认字段（schema.prisma L62-L66）：`archiveAsScreenshot/Monolith/PDF/Readable/WaybackMachine`
- Tag 可选字段（schema.prisma L206-L211）：同名但允许 null
...
相关代码：[archiveHandler.ts L233-L263](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/worker/lib/archiveHandler.ts#L233-L263)，[fetchHeaders.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/worker/lib/fetchHeaders.ts)
...
- 路径：[imageHandler.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/worker/lib/preservationScheme/imageHandler.ts)
...
- 路径：[pdfHandler.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/worker/lib/preservationScheme/pdfHandler.ts)
...
由 [handleScreenshotAndPdf.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/worker/lib/preservationScheme/handleScreenshotAndPdf.ts) 统一处理：
...
- 路径：[safeFetch.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/packages/lib/safeFetch.ts)
- 每一次请求都会通过 `assertUrlIsSafeForServerSideFetch` 做 SSRF 校验
- 自定义 DNS lookup，防止访问内网 IP
- 支持 HTTP/HTTPS/SOCKS 代理
- 最多跟随 5 次重定向
- 浏览器端的页面资源请求也通过 `protectPageRequests` 做了同样的 SSRF 拦截（[protectPageRequests.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/worker/lib/protectPageRequests.ts)）
...
format → suffix 的映射在 [getSuffixFromFormat.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/web/lib/shared/getSuffixFromFormat.ts)。
...
- 核心函数：
  - [createFile.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/packages/filesystem/createFile.ts)：写文件，自动 mkdir
  - [readFile.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/packages/filesystem/readFile.ts)：读文件 + 根据扩展名推断 Content-Type
  - [removeFile.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/packages/filesystem/removeFile.ts)：删文件（S3 DeleteObject 或 fs.unlink）

### 4.3 DB 字段（Link 模型）

在 [schema.prisma L166-L198](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/packages/prisma/schema.prisma#L166-L198) 中：

| 字段 | 类型 | 含义 | 状态值 |
|---|---|---|---|
| `type` | String | 链接类型 | `url` / `pdf` / `image` |
| `preview` | String? | 缩略图路径 | `null`（待生成）/ 路径字符串 / `"unavailable"`（生成失败/不支持） |
...
| `lastPreserved` | DateTime? | 最近一次归档时间戳 | `null`（待处理）/ 时间戳 |
| `indexVersion` | Int? | 搜索索引版本（归档成功后置 null 触发重新索引） | — |
...
### 5.1 层级一：直接 API 访问（`/api/v1/archives/[linkId]`）

文件路径：[[linkId].ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/web/pages/api/v1/archives/[linkId].ts)
...
权限解析核心：[resolveAccessibleArchive.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/web/lib/api/archives/resolveAccessibleArchive.ts)，[getPermission.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/web/lib/api/getPermission.ts)

### 5.2 层级二：用户内容域名 + JWT Token（`/api/v1/preserved/view`）

文件路径：[view.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/web/pages/api/v1/preserved/view.ts)，[createPreservedFormatUrl.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/web/lib/api/preserved/createPreservedFormatUrl.ts)
...
### 5.3 上传权限（POST /api/v1/archives/[linkId]）
...
### 6.1 任务调度：公平轮询（getLinkBatchFairly）

路径：[getLinkBatchFairly.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/worker/lib/getLinkBatchFairly.ts)
...
路径：[linkProcessing.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/worker/workers/linkProcessing.ts)
...
无论归档成功或失败，[archiveHandler.ts L203-L229](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/worker/lib/archiveHandler.ts#L203-L229) 的 `finally` 块统一执行：

1. 清理 timeout
2. **重新从 DB 读取 Link**（确认用户没有在归档过程中删除链接）
3. 如果 Link 仍存在：
   - 写入 `lastPreserved = now`
   - 对所有仍为 null 的归档字段标记为 `"unavailable"`（表示该格式不可用或失败）
...
代码位置：
- [imageHandler.ts L10-L14](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/worker/lib/preservationScheme/imageHandler.ts#L10-L14)
- [pdfHandler.ts L9-L13](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/worker/lib/preservationScheme/pdfHandler.ts#L9-L13)
- [handleScreenshotAndPdf.ts L29-L35](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/worker/lib/preservationScheme/handleScreenshotAndPdf.ts#L29-L35) / L68-L74
- [generatePreview.ts L22-L34](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/packages/lib/generatePreview.ts#L22-L34)
...
- 删除单个 Link：[deleteLinkById.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/web/lib/api/controllers/links/linkId/deleteLinkById.ts) → 调用 `removeFiles(linkId, collectionId)`
...
1. **超限失败时无 DB 标记**：超限只打 `console.log`，DB 字段为 null 会在 finally 被标记为 "unavailable"，但用户无法区分是"文件超限"还是"页面无法访问"。
...
路径：[generatePreview.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/packages/lib/generatePreview.ts)
...
| 用户上传图片 | ✅ 生成 |
| HTML 页面归档 | ⚠️ 优先用 `og:image`；失败则用 Playwright `screenshot({ quality: 20 })` 拍一张 10MB 内的低质量图 |
...
4. **og:image 额外网络请求**：[handleArchivePreview.ts](file:///d:/fz/0601/solo-dogfeeding/code/88-linkwarden/apps/worker/lib/preservationScheme/handleArchivePreview.ts) 会对 og:image URL 做 SSRF 校验后导航过去取图，再 `goBack`，增加一次完整的页面往返。
