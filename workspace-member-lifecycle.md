...
步骤 4: 被邀请人点击邮件链接 /callback/email?token=...&email=...
    ...
    │  ║     → 扩容判定（此时 user.emailVerified === null）      ║
...
### 3.2 PrismaAdapter + NextAuth Email Provider 写入 emailVerified 的代码证据

#### 证据 1：signIn callback 读取的 user 对象 emailVerified 仍为 null

[[...nextauth].ts L1326-L1364](file:///d:/fz/0601/solo-dogfeeding/code/91-linkwarden/apps/web/pages/api/v1/auth/%5B...nextauth%5D.ts#L1326-L1364)：
...
**解读**：signIn callback 在 PrismaAdapter 内部 `updateUser` **之前**被调用。此时传入的 `user` 对象是 `adapter.getUserByEmail()` 的查询结果——被邀请用户的 `emailVerified` 仍为 `null`。扩容逻辑正是依赖这个"旧值"来判断是否是首次点击链接。

#### 证据 2：jwt callback trigger="signIn" 不处理 emailVerified

[[...nextauth].ts L1411-L1488](file:///d:/fz/0601/solo-dogfeeding/code/91-linkwarden/apps/web/pages/api/v1/auth/%5B...nextauth%5D.ts#L1411-L1488)：
...
async jwt({ token, trigger, user }) {
  ...
  if (trigger === "signUp") {
    // 仅 SSO 新注册（accounts.length > 0）才显式写入 emailVerified
    ...
    // Email Provider 点击链接走的是 trigger="signIn" 分支
    // 此处仅自动生成 username，不处理 emailVerified
...
**解读**：Email Provider 点击链接时触发的是 `trigger="signIn"`（不是 `signUp`，因为用户账号已通过 postUser 提前创建），所以 jwt callback **不会**写入 emailVerified。该字段完全由 PrismaAdapter 内部在 signIn 返回 true 之后的 `adapter.updateUser({ emailVerified: new Date() })` 写入。这是 `@auth/prisma-adapter` 的标准 Email Provider 行为。

#### 证据 3：member-onboarding 提交时 updateUserById 不处理 emailVerified

[updateUserById.ts L170-L218](file:///d:/fz/0601/solo-dogfeeding/code/91-linkwarden/apps/web/lib/api/controllers/users/userId/updateUserById.ts#L170-L218)：
...
const updatedUser = await prisma.user.update({
  ...
  data: {
    ...
    // ... 其他字段
    password: isInvited || (data.newPassword && data.newPassword !== "")
      ? newHashedPassword
      : undefined,
    // ↑ data 中没有 emailVerified 字段，不会更新
  },
});
```

**解读**：`prisma.user.update` 的 `data` 对象中完全没有 `emailVerified` 字段。member-onboarding 提交时只补写 `name` 和 `password`，因为 `emailVerified` 在用户点击邀请邮件链接的那一刻（PrismaAdapter 内部）就已经写入了。

#### 证据 4：Credentials Provider 强制要求 emailVerified

[[...nextauth].ts L116-L119](file:///d:/fz/0601/solo-dogfeeding/code/91-linkwarden/apps/web/pages/api/v1/auth/%5B...nextauth%5D.ts#L116-L119)：
...
**解读**：状态 A/B（未点击链接或已点击未设密码）均无法通过 Credentials 登录——状态 A 连 `emailVerified` 都没有；状态 B 虽然 `emailVerified` 已写入但 `password` 为 `null`，bcrypt 校验仍会失败。

#### 证据 5：API verifyUser 中间件同样要求 emailVerified

[verifyUser.ts L49-L58](file:///d:/fz/0601/solo-dogfeeding/code/91-linkwarden/apps/web/lib/api/verifyUser.ts#L49-L58)：

```typescript
if (!user.emailVerified && process.env.NEXT_PUBLIC_EMAIL_PROVIDER === "true") {
  res.status(401).json({
    response: "Email not verified, please verify your email to continue using Linkwarden.",
  });
  return null;
}
```

### 3.3 emailVerified 写入时机（核准总结）
...
**关键核准结论**：
- **PrismaAdapter 的 Email Provider**（无论是 `id="email"` 或 `id="invite"`）在 **signIn callback 返回 `true` 后**，于 `adapter.updateUser({ emailVerified: new Date() })` 写入。
- signIn callback 执行时读取到的 `user.emailVerified` 仍为 `null`——这是正确的，因为扩容判断正是在更新前的用户对象上进行。
- member-onboarding 页面提交时，数据库中 `emailVerified` **已经有值**，只需补写 name 和 password。

### 3.4 Seat 自动扩容与激活对齐（核准）

signIn callback 中的扩容逻辑 [[...nextauth].ts L1326-L1364](file:///d:/fz/0601/solo-dogfeeding/code/91-linkwarden/apps/web/pages/api/v1/auth/%5B...nextauth%5D.ts#L1326-L1364)：
...
| `!email?.verificationRequest` | 点击链接（不是"发送邮件"请求） | 排除 `signIn("invite")` 发邮件时 |
...
- **令牌有效期**：[[...nextauth].ts L141](file:///d:/fz/0601/solo-dogfeeding/code/91-linkwarden/apps/web/pages/api/v1/auth/%5B...nextauth%5D.ts#L141) `maxAge: 1200` → **20 分钟**
...
[updateUserById.ts L170-L177](file:///d:/fz/0601/solo-dogfeeding/code/91-linkwarden/apps/web/lib/api/controllers/users/userId/updateUserById.ts#L170-L177)：
...
用户存在于 DB 中，但不能登录：
- Credentials Provider：`password === null` → bcrypt 校验失败
...
| 字段 | 值 |
|------|----|
| `emailVerified` | `new Date()`（PrismaAdapter 写入） |
| `name` | `null` |
| `password` | `null` |
| `parentSubscriptionId` | 订阅 ID |
| `isInvited` | `true` |
...
|----------|-------------------|----------|
| Credentials（密码） | `emailEnabled && !user.emailVerified` → 抛 `"Email not verified."` | [[...nextauth].ts L117-L119](file:///d:/fz/0601/solo-dogfeeding/code/91-linkwarden/apps/web/pages/api/v1/auth/%5B...nextauth%5D.ts#L117-L119) |
| Email Provider（`id=email` / `id=invite`） | PrismaAdapter 验证 token 后自动写入 `emailVerified` | `@auth/prisma-adapter` 内部 |
| SSO Provider | jwt callback `trigger="signUp"` 时显式写入 | [[...nextauth].ts L1427-L1453](file:///d:/fz/0601/solo-dogfeeding/code/91-linkwarden/apps/web/pages/api/v1/auth/%5B...nextauth%5D.ts#L1427-L1453) |
| API verifyUser 中间件 | `NEXT_PUBLIC_EMAIL_PROVIDER && !user.emailVerified` → 401 | [verifyUser.ts L49-L58](file:///d:/fz/0601/solo-dogfeeding/code/91-linkwarden/apps/web/lib/api/verifyUser.ts#L49-L58) |
...
| 时机 | 行为 | 代码 |
|------|------|------|
| 邀请成员**首次**接受邀请（点击链接 signIn 时 `emailVerified === null`） | 已验证 child users + 2 > quantity → **自动扩容** `updateSeats(stripeId, 新数量)` | [[...nextauth].ts L1326-L1364](file:///d:/fz/0601/solo-dogfeeding/code/91-linkwarden/apps/web/pages/api/v1/auth/%5B...nextauth%5D.ts#L1326-L1364) |
| Owner 软移除成员 | 若成员 `emailVerified` → `updateSeats(-1)` | [deleteUserById.ts L93-L97](file:///d:/fz/0601/solo-dogfeeding/code/91-linkwarden/apps/web/lib/api/controllers/users/userId/deleteUserById.ts#L93-L97) |
| Child user 自删 | 若成员 `emailVerified` → `updateSeats(-1)` | [deleteUserById.ts L183-L192](file:///d:/fz/0601/solo-dogfeeding/code/91-linkwarden/apps/web/lib/api/controllers/users/userId/deleteUserById.ts#L183-L192) |
...
```
              ...
              │  PrismaAdapter 写入      │                         │ disconnect: parentSubscription
    ...
    └─────────┬──────────┘
     ...
     │                 │
     │ 自删             │ Owner 软移除
     │ prisma.user.delete│ disconnect: parentSubscription
     ▼                 │
┌────────────┐          │
│  [已删除]   │          ▼
│ CASCADE 删除│  ┌────────────────────┐
...
```
emailVerified 写入（从 null 到 Date）
        │
